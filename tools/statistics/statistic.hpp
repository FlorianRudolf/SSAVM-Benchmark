...
namespace viennamesh
{
  ...
  public:
    ...
    BinT sum_downwards(NumericT value) const
    {
      BinT tmp = 0;

      const_iterator bin_it = bin(value);
      for (const_iterator it = begin(); it != bin_it; ++it)
        tmp += it->second;
    ...
    BinT sum_upwards(NumericT value) const
    {
      BinT tmp = 0;

      const_iterator bin_it = bin(value);
      for (; bin_it != end(); ++bin_it)
        tmp += bin_it->second;
    ...
    std::size_t bin_count() const { return bins.size(); }
...
//         ordered_values_.insert( cell_metric );
    ...
    void operator()(NumericT value)
    {
      ...
      sum_ += value;
      histogram_.increase( value );
      ordered_values_.insert( value );
      ++count_;
    }
